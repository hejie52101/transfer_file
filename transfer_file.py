...
def ssh_stby(ip, username, password, ne_partition):
    ...
    if username != "npti_sp":
        ...
        if "169.254.1.2" in rst_arp:
            act_mcp = "3"
            chan.send("telnet 169.254.1.2\n")
        elif "169.254.1.3" in rst_arp:
            act_mcp = "2"
            chan.send("telnet 169.254.1.3\n")
        else:
            ssh.close()
...
def sftp_func(ip, local_path, clear_cfg, superuser):
    ...
    if username != "npti_sp":
        ...
        localfile = glob.glob(a+"\\"+remotefile.split("/")[-1].split(".")[0]+"_?????.bin")
...
def sftp_transfer(ip, username, password, localfile, remotefile, mode="put"):
    ...
    if mode == "put":
        try:
            ...
            stdin, stdout, stderr = ssh.exec_command("mv " + remotefile + ".old " + remotefile)
            print(e)
            print("The script has rollback the version file.")
            sys.stdout.flush()
    else:
        sftp_t.get(remotefile, localfile)
    print("%s: Transfer version file to act mcp finished!" % threading.current_thread().name)
...
def sftp_func(ip, username, password, local_path, clear_cfg, mode):
    '''
    upgrade one NE
    '''
    # print "Run task %s (%s)..." % (ip, os.getpid())
    ...
    try:
        ...
        print("%s: sftp_func SSH connect failed. It will retry one time." % threading.current_thread().name)
        sys.stdout.flush()
        try:
            ssh.connect(ip, 22, username, password)
            print("%s: %s has login" % (threading.current_thread().name, ip))
    ...
    try:
        ...
        print("%s: Save old version file..." % threading.current_thread().name)
        sys.stdout.flush()
        stdin, stdout, stderr = ssh.exec_command("mv " + remotefile + " " + remotefile + ".old")
    except Exception as e:
        print("%s: Save old file SSH connect failed." % threading.current_thread().name)
        sys.stdout.flush()
        raise
    ...
    if sha_val in checksum:
        print("%s: checksum is OK" % threading.current_thread().name)
        sys.stdout.flush()
        stdin, stdout, stderr = ssh.exec_command("rm -f " + remotefile + ".old")
        ssh.close()
    else:
        print("%s: checksum is wrong" % threading.current_thread().name)
        sys.stdout.flush()
...
def sftp_thread(ip_list, username, password, version, clear_cfg, mode):
    print("Thread %s is running..." % threading.current_thread().name)
    sys.stdout.flush()
    try:
        for ip in ip_list:
            locals()["t_"+ip] = threading.Thread(target=sftp_func, args=(ip, username, password, version, clear_cfg, mode), name="Thread_" + ip)
            locals()["t_"+ip].start()
        for ip in ip_list:
            locals()["t_"+ip].join()
    except Exception as e:
...
if __name__ == '__main__':
    ...
    version = "CMCC-2018-12-21"    # 2018-05-20 V7.0.005-CMCC
    ...
    #     version_dir = os.path.join(r"\\Netstore-ch\r&d tn china\R&D_Server\Version Management\Dev_Version\Version to V&V\NPTI\V" + vv[0][0] + "." + vv[0][1], "V" + vv[0][0] + "." + vv[0][1] + "." + vv[0][2])
    # elif temp:
    #     version_dir = r"\\Netstore-ch\r&d tn china\R&D_Server\Version Management\Dev_Version\TempVersion\NPTI\V" + temp[0][0] + "." + temp[0][1]
    #     print(version_dir)
    #     ver_dir = ", ".join(os.listdir(version_dir))
    #     print(ver_dir)
    #     print(re.findall("("+".".join(temp[0])+r"\w*)", ver_dir))
    #     if re.findall("("+"".join(temp[0])+".*?),", ver_dir):
    #         version_dir = os.path.join(version_dir, re.findall("("+"".join(temp[0])+".*?),", ver_dir)[0])
    #     else:
    #         print("The version is not found, please type again!")
    #         sys.exit()
    # elif temp_cmcc:
    #     version_dir = r"\\Netstore-ch\r&d tn china\R&D_Server\Version Management\Dev_Version\TempVersion\NPTI\V" + temp[0][0] + "." + temp[0][1] + "-CMCC"
    #     ver_dir = ", ".join(os.listdir(version_dir))
    #     if re.findall("("+"".join(temp_cmcc[0])+".*?),", ver_dir):
    #         version_dir = os.path.join(version_dir, re.findall("("+"".join(temp_cmcc[0])+".*?),", ver_dir)[0])
    #     else:
    #         print("The version is not found, please type again!")
    #         sys.exit()
    # elif daily:
    #     version_dir = os.path.join(r"\\netstore-ch\R&D TN China\R&D_Server\Version Management\Dev_Version\DailyVersion\V7.5-NPTI", version)
    # elif daily_cmcc:
    #     version_dir = os.path.join(r"\\netstore-ch\R&D TN China\R&D_Server\Version Management\Dev_Version\DailyVersion\V7.5-NPTI-CMCC", version)
    # else:
    #     print("The version is not found, please type again!")
    if username == "root":
        password = "root"
        ...
        password = "admin1"
    elif username == "npti_sp":
        password = "sp&BAN42361"
    else:
        print("Incorrect username. Please retype!")
        sys.exit()
    sftp_thread(ip_list, username, password, version_dir, clear_cfg, mode)
"""